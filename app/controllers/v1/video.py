...
def get_all_tasks(request: Request, page: int = Query(1, ge=1), page_size: int = Query(10, ge=1)):
    ...
    # 单任务查询接口(get_task)会把素材路径转换成可访问的 /tasks/... URI，
    # 列表接口此前漏了同样的转换，前端拿到的是服务器本地绝对路径，浏览器无法播放。
    request_id = base.get_task_id(request)
    ...
    for task in tasks:
        ...
        if "videos" in task:
            response_task["videos"] = [
                _task_file_to_uri(v, endpoint, task_dir, request_id) for v in task["videos"]
            ]
        ...
        converted_tasks.append(response_task)
