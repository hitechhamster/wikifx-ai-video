...
def _parse_terms_response(response: str) -> List[str]:
    """从 LLM 回复里解析出字符串数组,容忍 ```json 围栏和前后多余文字。"""
    try:
...
def _generate_diversified_broll_terms(
    video_subject: str, video_script: str, amount: int
) -> List[str]:
    """
    "泛 b-roll"搜索词生成 —— 解决系列化产出时素材高度重复的问题。

    旧版 generate_terms 有一条约束"always add the main subject",逼着每个搜索词
    都带上 forex/dollar/euro,结果整条视频的搜索词高度雷同(forex dollar high /
    Euro slides forex ...),候选池极窄,不同视频反复撞同一批素材。

    财经新闻的画面不需要字面"外汇":交易员、证券交易所、城市金融区、银行大楼、
    点钞、电脑屏幕行情、报纸头条、办公室会议等真实拍摄 b-roll 都贴题。这里让模型
    针对当前这句脚本产出"具体、可拍摄、彼此不同"的画面词,显著扩大同源可用池。
    """
    prompt = f"""
# Role: Financial News B-roll Visual Director

## Goal
Generate exactly {amount} English stock-footage search terms for ONE sentence of a
financial-news voiceover. The terms drive a stock-video search; good terms describe
concrete, filmable real-world scenes — not abstract financial concepts.

## Rules
1. Return ONLY a json-array of strings, nothing else.
2. Each term is 2-4 words describing a CONCRETE, FILMABLE scene.
3. The FIRST term MUST be the most literal, on-the-nose visual for THIS sentence — the
   thing a viewer expects to see when hearing it. The other terms are varied backups.
   Examples of literal first terms:
     - "...comes down to Washington"      -> "white house aerial" or "us capitol building"
     - "analysts / traders / investors"   -> "trader at trading desk", "person analyzing stock charts"
     - "the euro" / "ECB"                 -> "euro banknotes", "european union flag building"
     - "the dollar / dollar strength"     -> "us dollar bills closeup", "dollar exchange rate board"
     - "Bank of Japan / yen"              -> "tokyo financial district", "japanese yen banknotes"
     - "stock / markets fell or rose"     -> "stock exchange trading floor", "red green stock ticker"
4. ICONIC LANDMARKS ARE ALLOWED if stock libraries actually have them: White House,
   US Capitol, Wall Street sign, New York Stock Exchange, Times Square, Tokyo/London/
   Frankfurt skylines, famous skyscrapers. Use them when the sentence clearly points
   there. ONLY avoid named things stock does NOT have (e.g. a specific central-bank
   boardroom interior, a named living official's face).
5. Do NOT overuse "press podium / microphones" — use it ONLY for a sentence about an
   official announcement, speech or rate decision, and at most once across all terms.
6. Prefer real filmed footage; avoid cartoon/animation/abstract motion-graphic concepts
   (e.g. avoid "growth arrow animation", "glowing data network").
7. English only.

## Output Example
["white house aerial view", "us dollar bills closeup", "trader at trading desk"]

## Context
Overall video topic: {video_subject}
This sentence: {video_script}
""".strip()
    ...
    for i in range(_max_retries):
        try:
            ...
            if "Error: " in response:
                logger.error(f"diversified broll terms failed: {response}")
                break
            terms = _parse_terms_response(response)
            if terms:
                logger.success(f"completed (b-roll): \n{terms}")
                return terms[:amount]
        except Exception as e:
            logger.warning(f"diversified broll terms attempt {i + 1} failed: {str(e)}")

    # 兜底:回退到普通 generate_terms(带 subject 的字面词),总比没有强
    logger.warning("diversified broll terms gave up, falling back to literal terms")
    return generate_terms(video_subject, video_script, amount=amount)
...
def generate_terms(
    ...
    diversify_broll: bool = False,
) -> List[str]:
    if diversify_broll:
        return _generate_diversified_broll_terms(
