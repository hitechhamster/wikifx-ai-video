...
def generate_audio(task_id, params, video_script):
    ...
    if not custom_audio_file:
        ...
        # 新闻播报腔(news_mode):只影响"喂给 TTS 模型的内容"，不改变 video_script
        # 本身——字幕/lower-third/ticker 都还是用原始文案，只有配音的语气变了。
        style_prompt = None
        if getattr(params, "news_mode", False):
            style_prompt = (
                "Deliver the following in an authoritative, measured, urgent "
                "breaking-news anchor tone, like a financial news broadcaster:"
            )
...
def get_video_materials(task_id, params, video_terms, audio_duration, video_script=None):
    """
    返回 (video_paths, clip_durations)。clip_durations 仅在编排器+按句子对齐模式下
    非空(每段画面应播出的秒数);其它路径返回 None,combine_videos 走老的快切填充。
    """
    # Orchestrator path: segment-aligned visual intent matching (M1+)
    if getattr(params, "use_orchestrator", False) and video_script and params.video_source != "local":
        logger.info("\n\n## orchestrator: segment-aligned material selection")
        from app.services import orchestrator
        try:
            paths, shot_log, clip_durations = orchestrator.orchestrate(
                task_id, params, video_script, audio_duration
            )
        except RuntimeError as e:
            # 硬性"绝不复用"约束触发：不要降级回旧的随机下载逻辑（那条路径
            # 本来就是要被取代的，会重新引入素材撞车），直接让任务失败，
            # 把原因报给用户，而不是悄悄拼一条有重复画面的视频。
            logger.error(f"orchestrator hard no-repeat constraint failed: {e}")
            sm.state.update_task(task_id, state=const.TASK_STATE_FAILED)
            return None, None
        if paths:
            return paths, clip_durations
        logger.warning("orchestrator returned empty paths, falling back to standard download")
...
def generate_final_videos(
    task_id, params, downloaded_videos, audio_file, subtitle_path, clip_durations=None
):
    final_video_paths = []
    ...
    for i in range(params.video_count):
        ...
        video.combine_videos(
            ...
            clip_speed_factor=getattr(params, "clip_speed_factor", 1.0),
            tense_transition_count=getattr(params, "tense_transition_count", 0),
            min_clip_duration=getattr(params, "min_clip_duration", None),
            random_clip_duration=getattr(params, "random_clip_duration", True),
...
def start(task_id, params: VideoParams, stop_at: str = "video"):
    """
    task_manager.run_task() 直接调用 func(*args, **kwargs)，没有任何 try/except
    包裹——任何没被下面分步逻辑预料到的异常(网络层 SSLError、Windows 文件占用
    PermissionError 等)都会让整条任务线程裸奔崩溃，任务状态永远卡在最后一次
    progress 不会变成 FAILED，前端轮询会一直转圈。这里统一兜底，保证任务状态
    总有一个终态。
    """
    try:
        return _start_impl(task_id, params, stop_at)
    except Exception:
        logger.exception(f"task {task_id} crashed with an unhandled exception")
        sm.state.update_task(task_id, state=const.TASK_STATE_FAILED)
        raise
...
def _start_impl(task_id, params: VideoParams, stop_at: str = "video"):
    ...
    downloaded_videos, clip_durations = get_video_materials(
    ...
    # 5b. BGM selection (M3 mood-driven, or M5 manual override)
    bgm_mode = getattr(params, "bgm_mode", "auto")
    if bgm_mode == "manual":
        # 用户在前端直接选了一首曲子，params.bgm_file 已经是目标路径。
        # 这里必须完全跳过下面的自动选曲分支——之前 use_mood_bgm 那条逻辑
        # 是无条件覆盖 params.bgm_file 的，如果手动指定和自动选曲都生效，
        # 手动选择会被自动选曲悄悄顶掉，等于"手动指定"形同虚设。
        ...
        if getattr(params, "force_tense_bgm", False):
            ...
            # 能用，缓解 tense 标签曲库太薄导致的批量同质化)。energy 上限可配，
            # 用户觉得 energy=5 那批太吵时可以把池子压到比如 3~4。
    ...
    final_video_paths, combined_video_paths = generate_final_videos(
        task_id, params, downloaded_videos, audio_file, subtitle_path, clip_durations
    )
