...
def tts(
    ...
    if is_azure_v2_voice(voice_name):
        ...
        if len(parts) >= 2:
            ...
            voice = voice_with_gender.split("-")[0]
            return gemini_tts(text, voice, voice_rate, voice_file, voice_volume, style_prompt=style_prompt)
        else:
        ...
        # 格式: openai:voice-Gender
        parts = voice_name.split(":")
        if len(parts) >= 2:
            voice_with_gender = parts[1]
            voice = voice_with_gender.split("-")[0]
            return openai_tts(text, voice, voice_rate, voice_file, voice_volume)
        else:
...
def _raw_pcm_to_mp3(pcm_bytes: bytes, sample_rate: int, channels: int, output_file: str) -> bool:
    """
    把裸 PCM(s16le) 编码成 mp3，直接走 ffmpeg subprocess，不用 pydub。

    pydub 依赖标准库 `audioop`，Python 3.13 已移除该模块，pydub 在这个
    解释器上 import 就会失败(见 M3 bgm_library.py 的同类问题)。Gemini TTS
    返回的就是裸 PCM，所以这里复用项目里 generate_silent_audio() 已经验证
    过的"直接调用 ffmpeg 二进制"模式，而不是引入新的坏依赖。
    """
    ensure_file_path_exists(output_file)
    ffmpeg_binary = utils.get_ffmpeg_binary()
    command = [
    ...
    if result.returncode != 0:
        ...
        logger.error(f"failed to convert PCM to mp3: {stderr.strip()}")
        return False
    return os.path.exists(output_file) and os.path.getsize(output_file) > 0


def apply_voice_rate_postprocess(audio_file: str, voice_rate: float) -> bool:
    """
    用 ffmpeg atempo 滤镜对已经生成好的音频文件做变速(不变调)，原地替换。

    给"TTS provider 本身不支持调速参数"的情况(目前是 gemini_tts；MiMo 同样
    不支持，但当前未配置/未使用，暂不在这里接入)用的统一后处理方案——和
    具体 TTS provider 无关，只要音频文件已经生成好就能用。

    atempo 单次调用的有效范围是 0.5~2.0，项目里 1.2~1.5 的语速区间在范围内，
    不需要链式叠加多个 atempo。

    Returns True 表示变速成功且文件已原地替换；False 表示失败，调用方应该
    保留原始(未变速)音频，而不是留下一个损坏的文件。
    """
    rate = voice_rate or 1.0
    if abs(rate - 1.0) < 1e-3:
        return True  # 不需要变速

    if not (0.5 <= rate <= 2.0):
        logger.warning(
            f"voice_rate {rate} 超出 ffmpeg atempo 单次调用的有效范围(0.5~2.0)，跳过变速"
        )
        return True

    tmp_file = f"{audio_file}.atempo_tmp.mp3"
    ffmpeg_binary = utils.get_ffmpeg_binary()
    command = [
        ffmpeg_binary,
        "-y",
    ...
    if result.returncode != 0 or not os.path.exists(tmp_file) or os.path.getsize(tmp_file) <= 0:
        stderr = (result.stderr or b"").decode(errors="ignore")
        logger.error(f"voice_rate atempo postprocess failed: {stderr.strip()}")
        if os.path.exists(tmp_file):
            os.remove(tmp_file)
        return False

    os.replace(tmp_file, audio_file)
    logger.info(f"applied atempo={rate} to {audio_file}")
    return True
...
def gemini_tts(
    ...
    用 google-genai(新 SDK，项目里 M2/M3 打标和 embedding 已经在用，架构
    统一)替换原来的 google.generativeai(旧 SDK，本项目未安装)。

    Args:
        text: 要转换的文本(也是字幕时间轴依据的原文，不能被 style_prompt 污染)
        voice_name: 语音名称，如 "Zephyr", "Puck" 等
        voice_rate: 语音速率(Gemini TTS 当前不支持显式调速参数，未使用；
                    真实变速走 apply_voice_rate_postprocess())
        voice_file: 输出音频文件路径
        voice_volume: 音频音量(当前未使用)
        style_prompt: 可选的语气/风格指令(比如"用新闻主播腔调播报")，只拼接到
                      *送给 TTS 模型* 的内容前面，不影响 text 本身——后面字幕
                      时间轴用的还是原始 text，否则字幕会把指令文字也算进去。
                      实测验证:5.5s 音频对应 13 词原文，指令本身没有被读出来，
                      说明 Gemini TTS 能正确把这种前缀当风格提示而不是待朗读内容。
    ...
    try:
        ...
        model = config.app.get("gemini_tts_model", "gemini-3.1-flash-tts-preview")

        logger.info(f"start, voice name: {voice_name}, model: {model}, styled: {bool(style_prompt)}")

        contents_for_synthesis = f"{style_prompt}\n\n{text}" if style_prompt else text

        response = client.models.generate_content(
            model=model,
            contents=contents_for_synthesis,
        ...
        # Gemini TTS 返回 24kHz 单声道 16-bit 裸 PCM(audio/L16)。
        if not _raw_pcm_to_mp3(audio_bytes, sample_rate=24000, channels=1, output_file=voice_file):
            return None

        # Gemini TTS API 本身不支持调速参数，voice_rate 之前在这条路径上是
        # 空转的(配置了但完全没生效)。这里用 ffmpeg atempo 做生成后变速，
        # 和 provider 无关——只要音频文件已经写好就能用同一套后处理。
        # 必须在变速*之后*再读时长、填字幕时间轴，否则字幕会按变速前的
        # 时长错位。
        if not apply_voice_rate_postprocess(voice_file, voice_rate):
            logger.warning(f"voice_rate postprocess failed, keeping original-speed audio")
...
def get_openai_voices() -> list[str]:
    ...
    Returns:
        声音列表，格式为 ["openai:alloy-Female", "openai:onyx-Male", ...]
    """
    voices_with_gender = [
        ("alloy", "Female"),
        ("echo", "Male"),
    ...
    return [f"openai:{voice}-{gender}" for voice, gender in voices_with_gender]
...
def openai_tts(
    ...
    OpenAI TTS API 原生支持 speed 参数(0.25~4.0)，是这几个 provider 里
    唯一能真正按 voice_rate 调速的，不用回退legacy整段填充时长比例。

    成本提醒(写进 CLAUDE.md 同步一份):tts-1 约 $15 / 1M 字符，一条
    1-2 分钟文案只有几分钱，批量铺量要留意总量。
    """
    try:
        ...
        base_url = config.app.get("openai_base_url", "") or None
        model = config.app.get("openai_tts_model", "tts-1")

        client = OpenAI(api_key=api_key, base_url=base_url)

        logger.info(f"start, voice name: {voice_name}, model: {model}, rate: {voice_rate}")
